...
class businessup_slider_widget extends WP_Widget {    
        ...
        <figure>
            <?php if( !empty($instance['image_uri']) ): ?>
                ...
                <?php else:
                    ...
                    echo '<img src="'.esc_url( get_template_directory_uri() ).'/images/slide/slide.jpg" class="img-responsive"/>'; 
	...
	?>
				...
				<td>
                    <label for="<?php echo $this->get_field_id('slider_title'); ?>"><?php esc_attr_e('Slider Title', 'businessup'); ?></label>
                </td>
				<td>
				...
				<td>
                    <label for="<?php echo $this->get_field_id('slider_desc'); ?>"><?php esc_attr_e('Slider Description', 'businessup'); ?></label>
                </td>
				<td>
        ...
        <table>
			<tr>
                <td style="width: 50%;">
                    <label for="<?php echo $this->get_field_id('btnone'); ?>"><?php esc_attr_e('Button One Label', 'businessup'); ?></label>
                </td>
                <td>
                    <label for="<?php echo $this->get_field_id('btnonelink'); ?>"><?php esc_attr_e('Button One Link', 'businessup'); ?></label>
                </td>
            </tr>
            <tr>
                <td>
            ...
            <tr>
                <td colspan="2">
                    <input type="checkbox" name="<?php echo $this->get_field_name('open_btnone_new_window'); ?>" id="<?php echo $this->get_field_id('open_btnone_new_window'); ?>" <?php if( !empty($instance['open_btnone_new_window']) ): checked( (bool) $instance['open_btnone_new_window'], true ); endif; ?> ><?php esc_attr_e( 'Open Button One Link in new window?','businessup' ); ?>
                </td>
            </tr>
        </table>
    <?php
