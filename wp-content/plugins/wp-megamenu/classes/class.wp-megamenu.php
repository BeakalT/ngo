...
function overrite_functions_wp_megamenu($args){
	...
	if (version_compare($wpmm_db_version, '1.1.1', '>')) {
		...
		$wpmm_wrap_class = '';
		if ($theme_id){
			$wpmm_wrap_class = "wp-megamenu-wrap {$menu_layout_class} {$menu_custom_class}";
		}
		...
		if (count($auto_intergration_menu)){
			...
			$item_wrap = '<div class="wpmm-fullwidth-wrap"></div><div class="wpmm-nav-wrap wpmm-main-wrap-'
			             .$args['theme_location'].'">' .$wpmm_on_mobile.' '.$brand_logo_wrap. '<ul id="%1$s"  data-wpmm_width="' . $wpmm_container_width . '" class="%2$s">%3$s</ul>'.$search_form.'</div>';
...
function wp_megamenu_nav_args($args = array()){
	...
	if ($theme_id){
		if (get_wpmm_theme_option('enable_sticky_menu', $theme_id) == 'true' && get_wpmm_theme_option('wpmm_orientation', $theme_id) !== 'wpmm_vertical'){
			$sticky_class = 'wpmm-sticky';
		}
		$menu_layout = get_wpmm_theme_option('menu_layout', $theme_id);
		if ($menu_layout){
